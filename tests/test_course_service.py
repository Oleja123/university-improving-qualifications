...
class CourseServiceCase(unittest.TestCase):
    ...
    def test_create(self):
        app.logger.info('Запуск тестирования создания курса')
    ...
    def test_update(self):
        app.logger.info('Запуск тестирования обновления курса')
        self.create_courses()
    ...
    def test_delete(self):
        app.logger.info('Запуск тестирования удаления курса')
        self.create_courses()
    ...
    def test_get_all(self):
        app.logger.info('Запуск тестирования получения всех курсов')
        self.create_courses()
    ...
    def test_get_all_paginated(self):
        app.logger.info('Запуск тестирования пагинации курсов')
        self.create_courses()
        page = course_service.get_all_paginated(page=1)
        self.assertEqual(page.total, 3)

    def test_same_name_create(self):
        app.logger.info('Запуск тестирования создания курса с одинаковым именем')
        self.create_courses()
        with self.assertRaises(ValueError):
    ...
    def test_change_included(self):
        app.logger.info('Запуск тестирования изменения флага included')
        self.create_courses()
    ...
    def test_get_by_id_nonexistent(self):
        app.logger.info('Запуск тестирования получения несуществующего курса по ID')
        with self.assertRaises(ValueError):
            course_service.get_by_id(999)

    def test_get_by_name_nonexistent(self):
        app.logger.info('Запуск тестирования получения несуществующего курса по имени')
        with self.assertRaises(ValueError):
            course_service.get_by_name('Non_Existent_Course')

    def test_create_without_course_type(self):
        app.logger.info('Запуск тестирования создания курса без типа')
        with self.assertRaises(ValueError):
            courseDTO = CourseDTO(name='Invalid Course')
            course_service.create(courseDTO)

    def test_get_all_empty(self):
        app.logger.info('Запуск тестирования получения курсов из пустой БД')
        self.assertEqual(len(course_service.get_all()), 0)

    def test_get_all_filtered(self):
        app.logger.info('Запуск тестирования фильтрации курсов')
        self.create_courses()
